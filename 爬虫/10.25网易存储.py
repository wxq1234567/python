        ...
        for i in cateList:
            ...
            for j in subCateGroupList:
                ...
                for k in categoryList:
                    ...
                    id = k['id']
                    link = f"https://you.163.com/item/list?categoryId={superCategoryId}&subCategoryId={id}"
                    a = self.get_xinxi(link)
                    # print(a)
                    categoryItemList = a["categoryItemList"]
                    for i in categoryItemList:
                        itemList = i["itemList"]
                        for j in itemList:
                            good = {}
                            good["name"] = j['name']
                            good["simpleDesc"] = j['simpleDesc']
                            good["retailPrice"] = j['retailPrice']
                            good["scenePicUrl"] = j['scenePicUrl'] + '?type=webp&imageView'
                            print(good)
                            self.tx.insert_one(good)
        # self.conn.close()
